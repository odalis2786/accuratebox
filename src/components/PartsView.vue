<template>
  <a-table :columns="columns" :dataSource="parts" rowKey="id">
...
const viewDetails = (parts) => {
  toolsStore.seleccionarParts(parts);
  console.log("Ver detalles de:", parts);
  router.push({ name: "detalles" });
};

onMounted(() => {
  toolsStore.fetchParts();
});
