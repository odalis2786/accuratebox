<template>
  <div v-if="tarea">
    ...
    <a-carousel
      v-if="tarea.imageUrls.length > 0"
      arrows
      :dots-class="'slick-dots slick-thumb'"
    >
      <template #customPaging="{ i }">
        <a>
          <img :src="tarea.imageUrls[i]" class="thumb-image" />
        </a>
      </template>
      <div v-for="(url, index) in tarea.imageUrls" :key="index">
        <img
          :src="url"
          :alt="`Imagen ${index + 1} del artículo ${tarea.name}`"
          class="article-image"
          @click="openModal(url)"
        />
      </div>
    ...
    >
      ...
      <a-descriptions-item label="Status">
        <a-tag :color="getPriorityColor(tarea.status)">
          {{ tarea.Status }}
        </a-tag>
      </a-descriptions-item>
...
const handleAddCommentOk = async () => {
  ...
  try {
    ...
    await database.addCommentToTareas(
      database.tareaSeleccionada.id,
      newCommentEntry
    );
    const idcoment = await database.tareaSeleccionada.id;
...
const tarea = computed(() => database.tareaSeleccionada);
...
const onIt = () => {
  Modal.confirm({
    ...
    cancelText: "No",
    onOk() {
      database.updateTareaStatus(tarea.value.id, "On It");
    },
    onCancel() {
...
const handleOkPendiente = async () => {
  if (!comentario.value.trim()) {
    ...
    try {
      await database.updateTareaStatus1(
        tarea.value.id,
        "Pendiente",
        comentario.value
      );
...
const cancelar = () => {
  Modal.confirm({
    ...
    onOk() {
      database
        .deleteWorkOrder(tarea.value.id)
        .then(() => {
          console.log("Orden de trabajo eliminada:", tarea.value.id);
        })
        .catch((error) => {
...
const cerrar = () => {
  Modal.confirm({
    ...
    onOk() {
      router.push({
        name: "CerrarOrden",
        params: { workOrderId: tarea.value.id },
      });
    },
    onCancel() {
      console.log("Operación cancelada");
...
const deleteComment = async (commentId) => {
  Modal.confirm({
    ...
    onOk() {
      database
        .deleteComment(commentId, database.tareaSeleccionada.id)
        .then(() => {
